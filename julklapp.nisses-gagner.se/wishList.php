...
<link href="wishlist.css" rel="stylesheet" type="text/css">	
	...
	<h1><?='�nskelistan'?></h1>
		...
		<?='bockat av, reserverat, p� din egen lista.'?>
	<?
	...
	?>
	<p><h2><?=$name.(endsWith($name,'s') ? "" : "s").' �nskelista'?></br>
	...
	<div id="div-1-main-left">
		...
		<table border=0 cellpadding=0 cellspacing=0 >
			<tr bgcolor=#f87820>
				...
				<td class=tabhead><img src=img/blank.gif width=200 height=6><br> <b><?='�nskan'?></b></td>
				<?
				if($logedinuser) {
	...
	while( $row=mysql_fetch_array($result) )
	{
		...
		if($logedinuser || $_REQUEST['redigera']=="true") {
			echo "<td class=tabval><a onclick=\"return confirm('Ta bort �nskning?');\" href=wishList.php?action=del&id=".$row['id']."><span class=red>[".'RADERA'."]</span></a></td>";
		} else {
			if($_SESSION['userid'] == $row['bought'] || $row['bought'] == 0) {
				echo "<td class=".$rowstyle."><a class='sel' onclick=\"return confirm('".$mark." ".$row['wish']." som reserverad?');\" href=wishList.php?action=choose&id=".$row['id']."&bought=".$row['bought']."><span class=".$rowcolor.">[".'K�PA'."]</span></a></td>";
			} else {
				echo "<td class=".$rowstyle."><span class=".$rowcolor.">[".'K�PA'."]</span></a></td>";
			}
		}
		...
		if($logedinuser || ($selectedUser['familyid'] == $_SESSION['userid'])) {
			echo "<td class=tabval><a onclick=\"return confirm('Redigera denna �nskning?\\n".$row["wish"]."')
	...
	if ($logedinuser) {
		echo "<small><a href='shoppingList.php'>".$name.(endsWith($name,'s') ? "" : "s")." ink�pslista</a></small>";
	...
	if($logedinuser || ($selectedUser['familyid'] == $_SESSION['userid'])) {
		...
		<form action=wishList.php name=edit method=get>
			...
			<table border=0 cellpadding=2 cellspacing=0>
				<tr>
					<td style="text-align:right">�nskan:</td><td><input class=edit type=text size=40 name=editWish value=<?=$wishToEdit['wish']?>></td>
				</tr>
				<tr>
					<td style="text-align:right">ev. L�nk:</td><td><input class=edit type=url size=40 name=editLink value=<?=$wishToEdit['link']?>></td>
				</tr>
				<tr>
		...
		<form action=wishList.php method=get>
			...
			<table border=0 cellpadding=2 cellspacing=0>
				<tr>
					<td style="text-align:right">�nskan:</td><td><input type=text size=40 name=wish></td>
				</tr>
				<tr>
					<td style="text-align:right">ev. L�nk:</td><td><input type=url size=40 name=link></td>
				</tr>
				<tr>
					<td></td><td><input type=submit border=0 name=wishlist value="L�gg till"></td>
				</tr>
			</table>
		</form>
